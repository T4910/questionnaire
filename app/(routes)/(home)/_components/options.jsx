...
const options = ({ list }) => {
  ...
  const Previous = () => {
    return (
        <Button
          onClick={prevOptions}
          className="absolute opacity-40 hover:opacity-90 rounded-full bg-neutral-900 -translate-y-1/2 top-1/2 -left-8 z-10 text-white p-2 size-7 !m-0">
          <FaChevronLeft />
        </Button>
    )
  }
  ...
  const Next = () => {
      return (
          <Button
            onClick={nextOptions}
            className="absolute opacity-40 hover:opacity-90 rounded-full bg-neutral-900 -translate-y-1/2 top-1/2 -right-8 z-10 text-white p-2 size-7 !m-0">
            <FaChevronRight />
          </Button>
      )
  }
